...
async function getFriendById(req, res) {
    let id = req.query.id
   let sql = 'select f.user_id as friend_id, f.online_status, f.remark, f.group_id, fg.name as group_name, f.room, f.unread_msg_count, u.username, u.avatar, u.phone, u.name, u.signature from friend as f join user as u on f.user_id = u.id join friend_group as fg on f.group_id = fg.id where f.id = ?'
